...
> **Windows用户**：将命令中的 `python` 替换为 `& D:\Software\Anaconda\envs\IOTA6910H\python.exe`，或在激活环境后使用。
...
#### 后门攻击参数
- `--target-class`: 后门目标类别（0-9，默认：0）
- `--base-class`: 要毒化的源类别（0-9，默认：1）
- `--poison-rate`: 毒化比例（推荐：0.005-0.03，即0.5%-3%）
- `--feature-steps`: 特征碰撞优化步数（默认：100）
- `--epsilon`: 最大扰动幅度（默认：16/255 ≈ 0.0627）
- `--feature-lambda`: 特征损失权重（默认：0.1）

#### Trigger参数
- `--trigger-size`: Trigger补丁大小（默认：5×5像素）
- `--trigger-value`: Trigger像素值（默认：1.0，白色）
- `--trigger-position`: Trigger位置（默认：bottom-right）
  - 可选：`bottom-right`, `top-left`, `top-right`, `bottom-left`

#### 训练参数
- `--epochs`: 训练轮数（默认：10）
- `--lr`: 学习率（默认：1e-3）
- `--batch-size`: 批次大小（默认：128）
- `--use-pretrained`: 使用ImageNet预训练模型

### 输出文件说明

后门攻击实验会在 `backdoor_results/` 目录下生成以下文件：

#### 1. 模型权重（`backdoor_model.pt`）
含有后门的ResNet-18模型检查点。

#### 2. 训练日志（`training_log.csv`）
记录每个epoch的训练指标。
...
#### 1. Clean Accuracy（干净准确率）
- 模型在正常测试样本上的分类准确率
- **预期值**: >85%
- **意义**: 高准确率证明后门隐蔽，不影响模型正常功能

#### 2. ASR（Attack Success Rate，攻击成功率）
- 带有trigger的测试样本被预测为目标类的比例
- **预期值**: >80%
- **意义**: 高ASR表示后门攻击有效且可靠
...
1. **固定随机种子**：默认 `seed=42`
2. **确定性算法**：使用固定的随机种子控制所有随机过程
3. **记录所有参数**：`metrics.json` 和 `results.json` 保存所有超参数
4. **保存模型权重**：训练的模型保存在相应目录
...
如果 `conda activate` 命令不工作，有两种解决方案：

**方案1：直接使用Python完整路径（推荐，最简单）**
```powershell
# 将所有命令中的 python 替换为完整路径
& D:\Software\Anaconda\envs\IOTA6910H\python.exe demo.py
& D:\Software\Anaconda\envs\IOTA6910H\python.exe run_experiment.py
```
...
# 步骤2：关闭当前PowerShell窗口

# 步骤3：重新打开PowerShell，然后就可以使用
conda activate IOTA6910H
python demo.py
```
